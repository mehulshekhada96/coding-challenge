...
export default function InputComponent({toDisplay, setToDisplay, setShowDownloadButton }) {
  ...
  return (
    ...
    <div className="flex items-start  justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 ">
      ...
      />
      {toDisplay.length ? null : (<span  className='text-sm text-gray-500'>Enter planets names (&apos;comma(&apos;,&apos;) separated&apos;). ex: Earth, Mercury, Mars, Venus, Saturn (Case Insensitive)</span>)} 
      </div>
      <button
        type="button"
